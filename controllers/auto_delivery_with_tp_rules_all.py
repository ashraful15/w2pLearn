...
#http://127.0.0.1:8000/welcome/auto_delivery_with_tp_rules_all/auto_delivery_start?depotid=170&cid=ZX
...
def auto_delivery_start():
    ...
    autDelCronRows=db((db.sm_depot.cid==c_id)&(db.sm_depot.depot_id==depotID)&(db.sm_depot.auto_del_cron_flag==0)).select(db.sm_depot.id,db.sm_depot.cid,limitby=(0,1))
    if autDelCronRows:
